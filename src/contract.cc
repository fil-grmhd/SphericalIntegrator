...
void precalc_sYlm(const CCTK_INT si, const int s, const int lmin, const int lmax)
{
   ...
   for (int si=0; si < nslices; ++si)
   {
      ...
      for (int l=lmin; l <= lmax; ++l)
         ...
         {
            // register
            sid_sYlm_re[si][lm+off] = SphericalIntegrator_RegisterSurfaceIntegral("sYlm_re", "none", si, 1, 1, "const");
            sid_sYlm_im[si][lm+off] = SphericalIntegrator_RegisterSurfaceIntegral("sYlm_im", "none", si, 1, 1, "const");
